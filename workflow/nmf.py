...
def try_k_range_nmf_argmax(X, kmin, kmax, random_state=42, max_iter=1000):
    """For each k (skipping k==2): fit NMF(n_components=k) -> assign labels = argmax(W)
       compute silhouette score on W. Return dict: k -> (silhouette, reconstruction_err)
    """
    n_samples = X.shape[0]
    results = {}
...
def main():
    ...
    results = try_k_range_nmf_argmax(X, args.kmin, args.kmax, random_state=args.random_state, max_iter=args.max_iter)
    ...
    if not valid_results:
        print('No successful results in the provided k range (excluding k=2). Adjust kmin/kmax or check input data.')
        sys.exit(1)
