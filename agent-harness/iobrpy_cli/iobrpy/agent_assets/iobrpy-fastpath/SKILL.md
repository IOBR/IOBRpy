...
20a. For `runall`, if the original raw FASTQ files are unavailable but a `fastp` output directory with cleaned FASTQ files still exists, that cleaned `fastp` result directory can be used as the `runall --fastq` input directory.
...
- If raw FASTQ is absent but a `fastp` cleaned FASTQ directory exists, that `fastp` result directory is also a valid `runall` input.
