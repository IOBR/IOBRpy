...
def anno_eset(eset_df: pd.DataFrame,
    ...
    # Base R merge(ordering) sorts by the join key (probe_id) with a stable order. Do the same
    # explicitly so later stable sorts on score preserve probe_id ordering for ties.
    ...
    if dups > 0:
        ...
        # keep highest scoring row per symbol; stable ascending sort on -score preserves
        # existing probe_id order for exact ties, mirroring base R order() behavior
        merged.sort_values(
            '_score',
            ascending=True,
            inplace=True,
