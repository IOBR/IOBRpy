...
- **bayesprism**
  - `-i/--input <CSV/TSV>` (required): genes × samples TPM matrix
  - `-o/--output <CSV/TSV>` (required): directory to save `theta.csv`, `theta_cv.csv`, and `Z_tumor.csv`.
  - `--threads`: CPU cores for BayesPrism (default 8).
  - `--sc_dat`: single-cell count matrix (cells × genes)
  - `--cell_state_labels`: A single-column CSV file, with cells ordered identically to --sc_dat, representing each cell’s state
  - `--cell_type_labels`: A single-column CSV file, with cells ordered identically to --sc_dat, representing each cell’s type
  - `--key`: One of the cell types in cell_type_label must be designated as tumor(This can be omitted when using the single-cell data provided by IOBRpy)
