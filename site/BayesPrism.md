...
- **What it computes / why it matters**: BayesPrism uses a Bayesian decomposition model to estimate per-sample cell-type proportions (tumor versus immune/stromal subtypes) and reconstruct tumor-specific expression matrices. The outputs support tumor microenvironment profiling, correlation with immune phenotypes or therapy response, and downstream differential or pathway analyses.
